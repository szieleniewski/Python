...
def calc(z, Ho, Om_m, Om_v, utype='general'):

    '''Cosmology calculator ala Ned Wright (www.astro.ucla.edu/~wright)
    input values = redshift, Ho, Omega_m, Omega_vac, universe_type=[general, open]
    ouput values = age at z, distance in Mpc, kpc/arcsec, apparent to abs mag conversion, luminosity distance (Mpc)

    '''
    
    if utype=='general':
        # if Univ is General, use Ho, Wm and given Wv
        H0 = Ho # Hubble constant
        WM = Om_m # Omega(matter)
        WV = Om_v # Omega(vacuum) or lambda
    elif utype == 'open':
        ...
        print "Choose open or general for utype"
        sys.exit()
    ...
    return (zage_Gyr, DCMR_Mpc, kpc_DA, (5*log10(DL_Mpc*1e6)-5), DL_Mpc)
