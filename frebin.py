...
def spaxel_scale_fits(fits_image, spaxel, new_file):
    ...
    x_newsize = x_field/float(spaxel[0])
    y_newsize = y_field/float(spaxel[1])
    ...
    head.update('CDELT1', spaxel[0], "mas")
    head.update('CDELT2', spaxel[1], "mas")
...
#Datacube Spaxel rescaling
def spaxel_scale(image, head, spaxel):
    '''Function that takes image and rebins it to chosen
    spaxel scale. Reads spaxel scale of input image
    from header keywords CDELT1/2 - UNITS MUST BE 'MAS'. 
    ...
    print 'X FIELD = ', x_field, ' mas'
    print 'Y FIELD = ', y_field, ' mas'

    x_newsize = x_field/float(spaxel[0])
    y_newsize = y_field/float(spaxel[1])

    print 'X NEWSIZE = ', x_newsize
    print 'Y NEWSIZE = ', y_newsize
